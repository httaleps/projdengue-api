...
public class Localizacao {
    ...
    @Size(max = 80, message = "A cidade deve ter no máximo {max} caracteres.")
    private String cidade;

    @Column(nullable = false, length = 2)
    @NotBlank(message = "A UF não pode estar vazia.")
    @Size(min = 2, max = 2, message = "A UF deve ter {max} caracteres.")
    private String uf;

    @Column(length = 9)
    @Size(max = 9, message = "O CEP deve ter no máximo {max} caracteres.")
